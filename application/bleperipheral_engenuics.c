...
static u8  uuid_type;                        /* UUID Type for BLEEngenuics service */ 
static u16 service_handle;                   /* Handle for the GATTS Service */
static ble_gatts_char_handles_t tx_handles;  /* TX Characteristic Handles */
static ble_gatts_char_handles_t rx_handles;  /* RX Characteristic Handles */          
static u16 conn_handle;                      /* Connection handle indicating if the device is GAP Connected to Client*/
static bool is_notification_enabled;         /* Flag to indicate if Notifications have been enabled by the Client */
...
static u32 BPEngenuicsAddService(void)
{
   ...
   err = sd_ble_uuid_vs_add(&ble_base_uuid, &uuid_type);
